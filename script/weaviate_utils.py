...
def list_collections(client):
    for collection_name in client.collections.list_all().keys():
        props = [
            (p.name, p.vectorizer, p.vectorizer_config.skip)
            for p in client.collections.list_all()[collection_name].properties
        ]
    ...
    if not client.collections.exists(collection_name):
        ...
        if stopwords is not None:
            ...
            collection.config.update(
                inverted_index_config=wvc.Reconfigure.inverted_index(stopwords_additions=stopwords)
            )
    # reload collection
