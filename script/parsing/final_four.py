...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        for row in rows:
            row_list.append([r.strip() for r in row.split("-- COL")])
    ...
    for i, d in data.iterrows():
        if i + 3 > data.shape[0]:
            ...
            (data.loc[i].row_type == "content")
            & (data.loc[i + 1].row_type == "table")
            & (data.loc[i + 2].row_type == "header")
        ):
            data.loc[i, "merge_"] = True
            data.loc[i, "merge_idx"] += f"{str(i)}"
            ante = 3
            while (
                (data.loc[i + ante].row_type == "content")
                & (data.loc[i + ante - 1].row_type == "header")
                & (data.loc[i + ante - 2].row_type == "table")
            ):
    ...
    data.drop(
        columns=[2, 8, 9, "merge_", "merge_idx", "len_merge_idx", "delete_"],
        inplace=True,
    )
    ...
    data.loc[
        data.level_1.str.contains("TITLE") | data.level_1.str.contains("Chapter"),
        "level_1",
    ] = "1"
